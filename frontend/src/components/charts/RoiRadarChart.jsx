...
function RoiRadarChart({ franchises }) {
  ...
  if (!franchises || franchises.length < 2) {
    return (
      <Box sx={{ height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Typography color="text.secondary">Select at least two franchises to compare.</Typography>
      </Box>
    );
  }

  const maxSpend = Math.max(...franchises.map((f) => f.lifetimeStats.totalSpent), 1);
  ...
  const metrics = [
    ...
    { name: 'Spend Efficiency', key: 'spend', norm: (f) => f.lifetimeStats.avgSpendEfficiency },
    { name: 'Titles', key: 'titles', norm: (f) => (f.titles / maxTitles) * 100 },
    {
      name: 'Auction Spend',
      key: 'auction',
        ...
        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={data}>
          ...
          {franchises.map((f, i) => (
            <Radar
              ...
              dataKey={f.shortName}
              stroke={
                f.color || (i === 0 ? theme.palette.primary.main : theme.palette.secondary.main)
              }
              fill={
                f.color || (i === 0 ? theme.palette.primary.main : theme.palette.secondary.main)
              }
