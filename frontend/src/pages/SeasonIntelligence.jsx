...
const getFranchiseColor = (shortName) => FRANCHISE_COLORS[shortName] || '#6366f1';
...
const SectionHeader = ({ icon: Icon, title, subtitle, color = 'primary.main', action }) => (
...
function SeasonIntelligence() {
  ...
  const { data, isLoading, isFetching, isPlaceholderData } = useSeasonIntelligence(selectedYear);
  const { data: compareData, isFetching: compareFetching } = useSeasonIntelligence(compareYear, {
  ...
  return (
    <Box sx={{ maxWidth: 1400, mx: 'auto', position: 'relative' }}>
        ...
        >
          <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
              Elite analytics and storylines for every IPL season
            </Typography>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <FormControlLabel
              ...
              sx={{ bgcolor: 'rgba(99,102,241,0.1)', pr: 2, pl: 1, py: 0.5, borderRadius: 8, m: 0 }}
            />
          </Box>
        </Box>
          ...
          {seasons?.map((year) => (
            <Chip
              ...
              sx={{
                ...
                cursor: 'pointer',
                transition: 'all 0.2s',
                bgcolor: selectedYear === year ? 'primary.main' : 'transparent',
                color: selectedYear === year ? 'white' : 'text.secondary',
                border: selectedYear === year ? 'none' : '1px solid rgba(148, 163, 184, 0.1)',
                '&:hover': {
                  bgcolor: selectedYear === year ? 'primary.dark' : 'rgba(99, 102, 241, 0.15)',
                  color: 'white',
                },
              }}
            />
      ...
      {data && (
        <Box sx={{ opacity: isPlaceholderData ? 0.6 : 1, transition: 'opacity 0.3s' }}>
                ...
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell
                        align="center"
                        sx={{ fontWeight: 900, color: 'primary.main', fontSize: '1.1rem' }}
                      >
                        IPL {selectedYear}
                      </TableCell>
                      <TableCell
                        align="center"
                        sx={{ fontWeight: 900, color: '#f59e0b', fontSize: '1.1rem' }}
                      >
                        IPL {compareYear}
                      </TableCell>
                    ...
                    {[
                      ...
                      return (
                        <TableRow key={row.key} hover>
                          ...
                          <TableCell align="center" sx={{ fontWeight: 900, color: diffColor }}>
                            {diff > 0 ? '+' : ''}
                            {diff.toLocaleString()}
                            {row.unit}
                          </TableCell>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5 }}>
                  <Box
                    sx={{
                      ...
                      alignItems: 'center',
                      justifyContent: 'center',
                      bgcolor: getFranchiseColor(data.champion.franchise.shortName),
                  ...
                  <Box>
                    <Typography
                      variant="overline"
                      sx={{ color: '#FFD700', fontWeight: 800, letterSpacing: 2 }}
                    >
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                      Won {data.champion.matchesWon} of {data.champion.matchesPlayed} matches{' '}
                      {data.runnerUp && ` • Defeated ${data.runnerUp.franchise.name} in the Final`}
                    </Typography>
                  </Box>
                </Box>
              </Box>
          ...
          <Grid container spacing={4} sx={{ mb: 4 }}>
            {[data, compareMode && compareData ? compareData : null]
              ...
              .map((d, dIdx) => {
                ...
                if (!d.seasonMvp && !aw?.finisher && !aw?.economyKing) return null;
                ...
                return (
                  ...
                  >
                    <SectionHeader
                      ...
                      subtitle="Elite analytics awards"
                      color="#f59e0b"
                    />
                    <Grid container spacing={2}>
                      {d.seasonMvp && (
                        <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                          <GlassCard
                            sx={{ borderTop: '3px solid #f59e0b', p: 2, cursor: 'pointer' }}
                            onClick={() => navigate(`/players/${d.seasonMvp.player.id}`)}
                          >
                            <Typography
                              variant="caption"
                      ...
                      {d.pressureIndex?.clutchBatter && (
                        <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                          <GlassCard
                            sx={{ borderTop: '3px solid #ec4899', p: 2, cursor: 'pointer' }}
                            onClick={() =>
                              navigate(`/players/${d.pressureIndex.clutchBatter.player.id}`)
                            }
                          >
                            <Typography
                              variant="caption"
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 1 }}>
                              <Avatar src={d.pressureIndex.clutchBatter.player.imageUrl} />
                              <Box>
                                <Typography
                                  variant="subtitle1"
                                  fontWeight={800}
                                ...
                                <Typography
                                  ...
                                  fontWeight={700}
                                >
                                  {d.pressureIndex.clutchBatter.runs} death/pressure runs
                                </Typography>
                              </Box>
                            </Box>
                          </GlassCard>
                      ...
                      {d.mostImprovedPlayer && (
                        <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                          <GlassCard
                            sx={{ borderTop: '3px solid #10b981', p: 2, cursor: 'pointer' }}
                            onClick={() => navigate(`/players/${d.mostImprovedPlayer.player.id}`)}
                          >
                            <Typography
                              variant="caption"
                              color="text.secondary"
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 1 }}>
                              <Avatar src={d.mostImprovedPlayer.player.imageUrl} />
                              <Box>
                                <Typography
                                  variant="subtitle1"
                                  fontWeight={800}
                                ...
                                <Typography
                                  ...
                                  fontWeight={700}
                                >
                                  + {d.mostImprovedPlayer.improvementScore} impact pts
                                </Typography>
                              </Box>
                            </Box>
                          </GlassCard>
                      ...
                      {aw?.economyKing && (
                        <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                          <GlassCard
                            sx={{ borderTop: '3px solid #3b82f6', p: 2, cursor: 'pointer' }}
                            onClick={() => navigate(`/players/${aw.economyKing.playerId}`)}
                          >
                            <Typography
                              variant="caption"
                              color="text.secondary"
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 1 }}>
                              ...
                              <Box>
                                <Typography
                                  ...
                                  sx={{ lineHeight: 1.2 }}
                                >
                                  {aw.economyKing.player?.name || 'Top Bowler'}
                                </Typography>
                                <Typography
                                  variant="caption"
                                  color="text.secondary"
                                  fontWeight={700}
                                >
                                  {aw.economyKing.economyRate} Econ (min 10 wkts)
                                </Typography>
                              </Box>
                            </Box>
                          </GlassCard>
                      ...
                      {aw?.finisher && (
                        <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                          <GlassCard
                            sx={{ borderTop: '3px solid #f97316', p: 2, cursor: 'pointer' }}
                            onClick={() => navigate(`/players/${aw.finisher.player.id}`)}
                          >
                            <Typography
                              variant="caption"
                              color="text.secondary"
                      ...
                      {aw?.enforcer && (
                        <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                          <GlassCard
                            sx={{ borderTop: '3px solid #8b5cf6', p: 2, cursor: 'pointer' }}
                            onClick={() => navigate(`/players/${aw.enforcer.player.id}`)}
                          >
                            <Typography
                              variant="caption"
                              color="text.secondary"
          ...
          <Grid container spacing={4} sx={{ mb: 4 }}>
            {[data, compareMode && compareData ? compareData : null]
              .filter(Boolean)
              .map((d, dIdx) => {
                if (!d.seasonNarrative && (!d.recordsCheck || d.recordsCheck.length === 0))
                  return null;
                return (
                  <Grid
                    item
                    ...
                    {d.seasonNarrative && (
                      ...
                      >
                        <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
                          <AutoGraphIcon sx={{ color: '#6366f1', fontSize: 32, mt: 0.5 }} />
                          <Box>
                            <Typography
                              variant="overline"
                              sx={{ color: '#6366f1', fontWeight: 800, letterSpacing: 1.5 }}
                            >
                    ...
                    {d.recordsCheck && d.recordsCheck.length > 0 && (
                      <Box>
                        <SectionHeader
                          ...
                          color={d.recordsCheck.some((r) => r.isBroken) ? '#ef4444' : '#94a3b8'}
                        />
                        <Grid container spacing={2}>
                            ...
                            >
                              ...
                              >
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                  ...
                                  <Typography
                                    variant="caption"
                                    color={rec.isBroken ? 'text.primary' : 'text.secondary'}
                                    fontWeight={800}
                                    textTransform="uppercase"
                                  >
                                    {rec.title}
                                ...
                                >
                                  <Typography
                                    variant="h5"
                                    fontWeight={900}
                                    sx={{ color: rec.isBroken ? '#ef4444' : 'text.primary' }}
                                  >
                                    {rec.new}{' '}
                                    {rec.isBroken && (
                                      <Typography
                                  ...
                                  <Typography
                                    variant="body2"
                                    color="text.secondary"
                                    sx={{ textDecoration: rec.isBroken ? 'line-through' : 'none' }}
                                  >
                                    {rec.isBroken
                                ...
                                {rec.player && (
                                  <Box
                                    ...
                                    onClick={() => navigate(`/players/${rec.player.id}`)}
                                  >
                                    <Avatar
              ...
              .map((d, dIdx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Grid container spacing={2}>
                      <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                        <GlassCard sx={{ borderTop: '3px solid #f59e0b', py: 2 }}>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            fontWeight={800}
                          ...
                          <Typography
                            ...
                            sx={{
                              mt: 0.5,
                              color: getFranchiseColor(d.storyline.highestTeamTotal?.team),
                            }}
                          >
                      ...
                      <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                        <GlassCard sx={{ borderTop: '3px solid #10b981', py: 2 }}>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            fontWeight={800}
                          ...
                          <Typography
                            ...
                            sx={{
                              mt: 0.5,
                              color: getFranchiseColor(d.storyline.biggestChase?.team),
                            }}
                          >
                      ...
                      <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                        <GlassCard sx={{ borderTop: '3px solid #ef4444', py: 2 }}>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            fontWeight={800}
                          ...
                          {d.storyline.closestMatch?.isSuperOver ? (
                            ...
                            <Typography
                              ...
                              sx={{
                                mt: 0.5,
                                color: getFranchiseColor(d.storyline.closestMatch?.winner),
                              }}
                            >
                      ...
                      <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                        <GlassCard sx={{ borderTop: '3px solid #3b82f6', py: 2 }}>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            fontWeight={800}
                          ...
                          <Typography
                            ...
                            sx={{
                              mt: 0.5,
                              color: getFranchiseColor(d.storyline.lowestTeamTotalBatFirst?.team),
                            }}
                          >
                      ...
                      <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                        <GlassCard
                          sx={{ borderTop: '3px solid #8b5cf6', py: 2, cursor: 'pointer' }}
                          onClick={() =>
                            navigate(`/players/${d.storyline.highestIndividualScore?.player?.id}`)
                          }
                        >
                          <Typography
                            variant="caption"
                      ...
                      <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                        <GlassCard
                          sx={{ borderTop: '3px solid #d946ef', py: 2, cursor: 'pointer' }}
                          onClick={() =>
                            navigate(`/players/${d.storyline.bestBowlingSpell?.player?.id}`)
                          }
                        >
                          <Typography
                            variant="caption"
                          ...
                          >
                            <Box>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                                ...
                                <Typography
                                  variant="overline"
                                  sx={{ color: '#f59e0b', fontWeight: 800, letterSpacing: 1 }}
                                >
                                  HALL OF FAME MATCH
                                </Typography>
                              </Box>
                              <Typography variant="h5" fontWeight={900} sx={{ mb: 0.5 }}>
                                {d.hallOfFameMatch.match.team1} vs {d.hallOfFameMatch.match.team2}
                              </Typography>
                              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                                {new Date(d.hallOfFameMatch.match.date).toLocaleDateString()} •{' '}
              ...
              .map((d, dIdx) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <Grid container spacing={2}>
                      <Grid item xs={6} md={compareMode && compareData ? 6 : 3}>
                        <GlassCard sx={{ textAlign: 'center', py: 2, px: 1 }}>
                        ...
                        >
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                            ...
                            <Typography variant="body2" fontWeight={900} color="success.main">
                              Avg Match Aggregate: {d.trends.highestScoringVenue?.avg}
                            </Typography>
                          </Box>
                          <Typography variant="body2" fontWeight={700} noWrap sx={{ mb: 2 }}>
                          ...
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                            ...
                            <Typography variant="body2" fontWeight={900} color="error.main">
                              Avg Match Aggregate: {d.trends.lowestScoringVenue?.avg}
                            </Typography>
                          </Box>
          ...
          <Grid container spacing={4} sx={{ mb: 4 }}>
            {[data, compareMode && compareData ? compareData : null]
              .filter(Boolean)
              .map((d, dIdx) => {
                const hasPressure = d.pressureIndex?.clutchBatter || d.pressureIndex?.clutchBowler;
                if (
                  !hasPressure &&
                  !d.mostImprovedPlayer &&
                  ...
                  >
                        ...
                        <Grid container spacing={2}>
                          {d.pressureIndex.clutchBatter && (
                            <Grid item xs={12} sm={6}>
                              <GlassCard
                                sx={{ borderTop: '3px solid #ec4899', cursor: 'pointer' }}
                                onClick={() =>
                                  navigate(`/players/${d.pressureIndex.clutchBatter.player.id}`)
                                }
                              >
                          ...
                          {d.pressureIndex.clutchBowler && (
                            <Grid item xs={12} sm={6}>
                              <GlassCard
                                sx={{ borderTop: '3px solid #ec4899', cursor: 'pointer' }}
                                onClick={() =>
                                  navigate(`/players/${d.pressureIndex.clutchBowler.player.id}`)
                                }
                              >
                                <Typography
                                  variant="caption"
                    ...
                    {d.mostImprovedPlayer && (
                      <Box sx={{ mb: 4 }}>
                        ...
                        />
                        <GlassCard
                          sx={{ borderLeft: '4px solid #10b981', cursor: 'pointer' }}
                          onClick={() => navigate(`/players/${d.mostImprovedPlayer.player.id}`)}
                        >
                          <Box
                            sx={{
                            ...
                            <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                              <Box>
                                ...
                                <Typography variant="h6" fontWeight={800} color="text.secondary">
                                  {d.mostImprovedPlayer.player.role === 'Bowler'
              ...
              .map((d, dIdx) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <Grid container spacing={2}>
                      <Grid item xs={12} sm={compareMode && compareData ? 6 : 4}>
                        <GlassCard sx={{ borderTop: '3px solid #f43f5e', py: 2 }}>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            fontWeight={800}
                          ...
                          {d.seasonMilestones.mostSixesInnings ? (
                            <>
                              ...
                              <Typography
                                ...
                                sx={{ mt: 0.5 }}
                              >
                                {d.seasonMilestones.mostSixesInnings.player.name} (
                                {d.seasonMilestones.mostSixesInnings.runs} runs)
                              </Typography>
                            </>
                          ) : (
                            <Typography
                      ...
                      <Grid item xs={12} sm={compareMode && compareData ? 6 : 4}>
                        <GlassCard sx={{ borderTop: '3px solid #8b5cf6', py: 2 }}>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            fontWeight={800}
                      ...
                      <Grid item xs={12} sm={compareMode && compareData ? 6 : 4}>
                        <GlassCard sx={{ borderTop: '3px solid #0ea5e9', py: 2 }}>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            fontWeight={800}
                      ...
                      <Grid item xs={12} sm={compareMode && compareData ? 6 : 4}>
                        <GlassCard sx={{ borderTop: '3px solid #14b8a6', py: 2 }}>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            fontWeight={800}
                          ...
                          {d.seasonMilestones.mostEconomicalSpell ? (
                            <>
                              ...
                              >
                                {d.seasonMilestones.mostEconomicalSpell.econ} Econ
                              </Typography>
                              <Typography
                                variant="body2"
                                color="text.secondary"
                                ...
                                sx={{ mt: 0.5 }}
                              >
                                {d.seasonMilestones.mostEconomicalSpell.player.name} (
                                {d.seasonMilestones.mostEconomicalSpell.figures} in 4 ovs)
                              </Typography>
                            </>
                          ) : (
                            <Typography
                      ...
                      <Grid item xs={12} sm={compareMode && compareData ? 6 : 4}>
                        <GlassCard sx={{ borderTop: '3px solid #d946ef', py: 2 }}>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            fontWeight={800}
                          ...
                          {d.seasonMilestones.mostDotsInnings ? (
                            <>
                              <Typography
                                ...
                                sx={{ mt: 0.5, color: '#d946ef' }}
                              >
                                {d.seasonMilestones.mostDotsInnings.dotBalls} Dots
                              </Typography>
                              <Typography
                                variant="body2"
                                color="text.secondary"
                                ...
                                sx={{ mt: 0.5 }}
                              >
                                {d.seasonMilestones.mostDotsInnings.player.name} (
          ...
          <Grid container spacing={4} sx={{ mb: 4 }}>
            {[data, compareMode && compareData ? compareData : null]
              .filter(Boolean)
              .map((d, dIdx) => {
                if (d.bestXI?.length !== 11) return null;
                return (
                  <Grid
                    item
                    xs={12}
                          ...
                          <TableBody>
                            {d.bestXI.map((p, i) => (
                              <TableRow
                                key={i}
                                hover
                                onClick={() => navigate(`/players/${p.player.id}`)}
                                sx={{ cursor: 'pointer' }}
                              >
                                <TableCell>
                                  <Chip
                                    size="small"
                                    label={p.role === 'Wicket-Keeper' ? 'WK' : p.role}
                                    sx={{
                                      fontWeight: 800,
                                      fontSize: '0.65rem',
                                      height: 20,
                                ...
                                <TableCell align="right">
                                  <Typography variant="body2" fontWeight={700}>
                                    {p.role === 'Batter' || p.role === 'Wicket-Keeper'
                                      ? `${p.totalRuns} runs`
                                      : p.role === 'Bowler'
                                        ? `${p.totalWickets} wkts`
          ...
          <Grid container spacing={4} sx={{ mb: 4 }}>
            {[data, compareMode && compareData ? compareData : null]
              ...
              .map((d, dIdx) => {
                if (
                  !d.clutchPerformers ||
                  (!d.clutchPerformers.playoffBatter && !d.clutchPerformers.playoffBowler)
                )
                  return null;
                return (
                  <Grid
                  ...
                  >
                    ...
                    <Grid container spacing={3}>
                      {d.clutchPerformers.playoffBatter && (
                        <Grid item xs={12} md={compareMode && compareData ? 12 : 6}>
                          <GlassCard
                            sx={{ borderLeft: '4px solid #ef4444', cursor: 'pointer' }}
                            onClick={() =>
                              navigate(`/players/${d.clutchPerformers.playoffBatter.player.id}`)
                            }
                          >
                            <Typography
                              variant="caption"
                      ...
                      {d.clutchPerformers.playoffBowler && (
                        <Grid item xs={12} md={compareMode && compareData ? 12 : 6}>
                          <GlassCard
                            sx={{ borderLeft: '4px solid #f59e0b', cursor: 'pointer' }}
                            onClick={() =>
                              navigate(`/players/${d.clutchPerformers.playoffBowler.player.id}`)
                            }
                          >
                            <Typography
                              variant="caption"
            ...
            {[data, compareMode && compareData ? compareData : null]
              ...
              .map((d, dIdx) => (
                ...
                >
                  ...
                  <Grid container spacing={3}>
                    {d.seasonMvp && (
                      <Grid item xs={12} md={compareMode && compareData ? 12 : 4}>
                        <GlassCard
                          sx={{
                            cursor: 'pointer',
                            background:
                            ...
                            '&:hover': { boxShadow: '0 0 32px rgba(59,130,246,0.2)' },
                          }}
                          onClick={() => navigate(`/players/${d.seasonMvp.player.id}`)}
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                            <Box
                              sx={{ p: 1, borderRadius: '50%', bgcolor: 'rgba(59,130,246,0.2)' }}
                            >
                            ...
                            <Box>
                              <Typography
                                variant="overline"
                                sx={{ color: '#3b82f6', fontWeight: 900, letterSpacing: 2 }}
                              >
                                🌟 Season MVP
                              </Typography>
                            </Box>
                      ...
                      <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                        <GlassCard
                          sx={{
                            ...
                            borderLeft: '4px solid #f59e0b',
                            '&:hover': { boxShadow: '0 0 32px rgba(245,158,11,0.2)' },
                          }}
                          onClick={() => navigate(`/players/${d.orangeCap.player.id}`)}
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                            <Box
                              sx={{ p: 1, borderRadius: '50%', bgcolor: 'rgba(245,158,11,0.2)' }}
                            >
                            ...
                            <Box>
                              <Typography
                                variant="overline"
                                sx={{ color: '#f59e0b', fontWeight: 900, letterSpacing: 2 }}
                              >
                                🧡 Orange Cap
                              </Typography>
                            </Box>
                          ...
                          <Grid container spacing={2}>
                            {[
                              { label: 'Runs', value: d.orangeCap.runs, color: '#f59e0b' },
                              {
                                label: 'Average',
                              ...
                              <Grid item xs={6} key={i}>
                                <Box sx={{ textAlign: 'center' }}>
                                  <Typography variant="h6" fontWeight={900} sx={{ color: s.color }}>
                                    {s.value}
                                  </Typography>
                                  <Typography
                                    variant="caption"
                      ...
                      <Grid item xs={12} sm={6} md={compareMode && compareData ? 6 : 4}>
                        <GlassCard
                          sx={{
                            ...
                            '&:hover': { boxShadow: '0 0 32px rgba(156,39,176,0.2)' },
                          }}
                          onClick={() => navigate(`/players/${d.purpleCap.player.id}`)}
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                            <Box
                              sx={{ p: 1, borderRadius: '50%', bgcolor: 'rgba(156,39,176,0.2)' }}
                            >
                            ...
                            <Box>
                              <Typography
                                variant="overline"
                                sx={{ color: '#9c27b0', fontWeight: 900, letterSpacing: 2 }}
                              >
                                💜 Purple Cap
                              </Typography>
                            </Box>
                          ...
                          <Grid container spacing={2}>
                            {[
                              { label: 'Wickets', value: d.purpleCap.wickets, color: '#9c27b0' },
                              {
                                label: 'Economy',
            ...
            {[data, compareMode && compareData ? compareData : null]
              ...
              .map((d, dIdx) => (
                ...
                >
                  ...
                  <Grid container spacing={3}>
                    <Grid item xs={12} lg={compareMode && compareData ? 12 : 6}>
                      <GlassCard sx={{ p: 0, overflow: 'hidden' }}>
                        <Box
                          sx={{ p: 2.5, pb: 1.5, borderBottom: '1px solid rgba(148,163,184,0.08)' }}
                        >
                          <SectionHeader
                            ...
                            <TableBody>
                              {d.topBatters?.map((b, i) => (
                                ...
                                >
                                  ...
                                  <TableCell>
                                    <Typography variant="body2">{b.average?.toFixed(1)}</Typography>
                                  </TableCell>
                                  <TableCell>
                            ...
                            <TableBody>
                              {d.topBowlers?.map((b, i) => (
                                ...
                                >
                                  ...
                                  <TableCell>
                                    <Typography variant="body2" fontWeight={800} color="#9c27b0">
                                      {b.wickets}
                                    </Typography>
                                  </TableCell>
                                  <TableCell>
          ...
          <Grid container spacing={4} sx={{ mb: 4 }}>
            {[data, compareMode && compareData ? compareData : null]
              .filter(Boolean)
              .map((d, dIdx) => {
                if (!d.franchiseStandings || d.franchiseStandings.length === 0) return null;
                return (
                  <Grid
                    item
                    xs={12}
                    ...
                    <GlassCard sx={{ p: 0, overflow: 'hidden' }}>
                      <TableContainer sx={{ overflowX: 'auto' }}>
                        <Table size="small">
                          <TableHead>
                            <TableRow>
                              {[
                                ...
                                'L',
                                'Win %',
                                ...(d.hasAuctionData && (!compareMode || !compareData)
                                  ? ['Spent', 'ROI']
                                  : []),
                              ].map((h) => (
                                <TableCell
                          ...
                          <TableBody>
                            {d.franchiseStandings.map((f, i) => (
                              <TableRow
                                key={f.franchise.id}
                                sx={{
                                  bgcolor: f.isChampion ? 'rgba(255,215,0,0.08)' : 'transparent',
                              ...
                              >
                                  ...
                                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                    <Box
                                      sx={{
                                        ...
                                        height: 28,
                                        borderRadius: 4,
                                        bgcolor: getFranchiseColor(f.franchise.shortName),
                                      }}
                                    />
                                    <Typography variant="body2" fontWeight={800}>
                                      {f.franchise.name}{' '}
                                ...
                                <TableCell>
                                  <Typography variant="body2">{f.matchesPlayed}</Typography>
                                </TableCell>
                                <TableCell>
