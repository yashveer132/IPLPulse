...
function PlayerValueRankings() {
  ...
  const columns = [
    ...
    { id: 'bat', label: 'Batting', render: (_, row) => Math.round(row.battingValueScore) },
    { id: 'bowl', label: 'Bowling', render: (_, row) => Math.round(row.bowlingValueScore) },
    { id: 'cons', label: 'Consistency', render: (_, row) => Math.round(row.consistencyScore) },
    {
      id: 'total',
      label: 'Value Score',
      render: (_, row) => (
        <Chip label={Math.round(row.lifetimeValueScore)} color="primary" sx={{ fontWeight: 800 }} />
      ),
    },
  ];
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" mb={4}>
        Proprietary ranking of IPL players based on batting impact, bowling economy, match
      ...
      <DataTable
        ...
        data={data?.players}
        isLoading={isLoading}
        onRowClick={(row) => navigate(`/analytics/player-value/${row.playerId}`)}
