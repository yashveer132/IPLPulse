...
export async function getSeasonLeaderboards() {
  ...
  const [mostRuns, mostWickets, mostSixes, highestStrikeRate, bestEconomy, highestScore] =
    await Promise.all([
  ...
  return { mostRuns, mostWickets, mostSixes, highestStrikeRate, bestEconomy, highestScore };
}
...
async function computeDynamicRecord(categoryId, recordId) {
  ...
  switch (recordId) {
    ...
    case 'career-100s': {
      ...
      return await formatPlayerStats(stats.slice(0, limit), prisma, '_count', 'playerId');
    }
    ...
    case 'career-strike-rate': {
      ...
      srStats.forEach((s) => {
        s.strikeRate = s._sum.ballsFaced > 0 ? (s._sum.runsScored / s._sum.ballsFaced) * 100 : 0;
      });
      srStats.sort((a, b) => b.strikeRate - a.strikeRate);
      return await formatPlayerStats(srStats.slice(0, limit), prisma, '', 'strikeRate');
    }
    ...
    case 'career-average': {
      ...
      avgStats.forEach((s) => {
        ...
        s.average = dismissals > 0 ? s._sum.runsScored / dismissals : s._sum.runsScored;
      });
      avgStats.sort((a, b) => b.average - a.average);
      return await formatPlayerStats(avgStats.slice(0, limit), prisma, '', 'average');
    }
    ...
    case 'highest-score':
      return await prisma.playerMatchStats
        .findMany({
          ...
          take: limit,
          select: { playerId: true, runsScored: true, ballsFaced: true, season: true, team: true },
        })
        .then((res) =>
          formatRawMatchStats(
            res,
            prisma,
            'runsScored',
            (m) => `${m.runsScored} off ${m.ballsFaced} balls for ${m.team} (${m.season})`
          )
        );
      ...
      return await prisma.playerMatchStats
        ...
        .then((res) =>
          formatRawMatchStats(
            ...
            prisma,
            'sixes',
            (m) => `${m.sixes} sixes (${m.runsScored} runs) for ${m.team} (${m.season})`
          )
        );
    ...
    case 'fastest-50': {
      ...
      return await formatRawMatchStats(
        ...
        prisma,
        'sr',
        (m) => `${m.runsScored} off ${m.ballsFaced} for ${m.team} (${m.season})`
      );
    }
    ...
    case 'boundary-runs': {
      ...
      return await formatPlayerStats(bStats.slice(0, limit), prisma, '', 'totalBoundaryRuns');
    }
    ...
    case 'seasons-500': {
      ...
      return await formatPlayerStats(s500.slice(0, limit), prisma, '_count', 'playerId');
    }
    ...
    case 'consecutive-30s': {
      ...
      const allStats = await prisma.playerMatchStats.findMany({
        select: { playerId: true, runsScored: true, match: { select: { date: true } } },
      });

      const playerLogs = {};
      allStats.forEach((s) => {
        if (!playerLogs[s.playerId]) playerLogs[s.playerId] = [];
        playerLogs[s.playerId].push({ runs: s.runsScored, date: new Date(s.match.date).getTime() });
      });
    ...
    case 'sixes-per-innings': {
      ...
      sStats.forEach((s) => {
        s.sixPerInns = s._count.playerId > 0 ? s._sum.sixes / s._count.playerId : 0;
      });
      sStats.sort((a, b) => b.sixPerInns - a.sixPerInns);
      return await formatPlayerStats(sStats.slice(0, limit), prisma, '', 'sixPerInns');
    }
    ...
    case 'six-frequency': {
      ...
      freqStats.forEach((s) => {
        s.sixFreq = s._sum.ballsFaced > 0 ? (s._sum.sixes / s._sum.ballsFaced) * 100 : 0;
      });
      freqStats.sort((a, b) => b.sixFreq - a.sixFreq);
      return await formatPlayerStats(freqStats.slice(0, limit), prisma, '', 'sixFreq');
    }
    ...
    case 'boundary-percentage': {
      ...
      boundaryStats.forEach((s) => {
        ...
        s.boundaryPercentage = s._sum.runsScored > 0 ? (boundaryRuns / s._sum.runsScored) * 100 : 0;
      });
    ...
    case 'death-sr': {
      const dSrStats = await prisma.playerCrazyStats.findMany({
        ...
        select: { playerId: true, deathOversRunsScored: true, deathOversBallsFaced: true },
      });
      dSrStats.forEach((s) => {
      ...
      return await formatRawMatchStats(
        ...
        prisma,
        'deathSR',
        (s) => `${s.deathOversRunsScored} runs off ${s.deathOversBallsFaced} balls`
      );
    }
    ...
    case 'career-5w': {
      ...
      return await formatPlayerStats(w5Stats.slice(0, limit), prisma, '_count', 'playerId');
    }
    ...
    case 'powerplay-wickets':
      return await prisma.playerCrazyStats
        ...
        .then((res) =>
          formatRawMatchStats(res, prisma, 'powerplayWickets', () => 'Wickets in Overs 1-6')
        );
    ...
    case 'venue-runs':
      return await prisma.venueMasteryStat
        ...
        .then((res) => formatRawMatchStats(res, prisma, 'runsScored', (m) => m.venue));
    ...
    case 'venue-wickets':
      return await prisma.venueMasteryStat
        ...
        .then((res) => formatRawMatchStats(res, prisma, 'wickets', (m) => m.venue));
    ...
    case 'venue-sixes':
      return await prisma.venueMasteryStat
        ...
        .then((res) => formatRawMatchStats(res, prisma, 'sixes', (m) => m.venue));

    case 'venue-sr': {
      const vSrStats = await prisma.venueMasteryStat.findMany({
        where: { ballsFaced: { gte: 200 } },
        select: { playerId: true, runsScored: true, ballsFaced: true, venue: true },
      });
      vSrStats.forEach((s) => {
    ...
    case 'silent-assassin': {
      ...
      silentStats.forEach((s) => {
        ...
        s.nonBoundaryPercentage =
          s._sum.runsScored > 0 ? (nonBoundaryRuns / s._sum.runsScored) * 100 : 0;
      });
      silentStats.sort((a, b) => b.nonBoundaryPercentage - a.nonBoundaryPercentage);
      return await formatPlayerStats(
    ...
    case 'one-man-army': {
      ...
      const oneManStats = allSeasons
        .map((s) => {
          ...
          const percentage = teamTotal > 0 ? (s.totalRuns / teamTotal) * 100 : 0;
          return {
            playerId: s.playerId,
            percentage,
            runs: s.totalRuns,
    ...
    case 'nervous-90s': {
      ...
      return await formatPlayerStats(n90s.slice(0, limit), prisma, '_count', 'playerId');
    }
    ...
    case 'dot-ball-survivor': {
      ...
      dotStats.forEach((s) => {
        s.dotPercentage = s._sum.ballsFaced > 0 ? (s._sum.dotBalls / s._sum.ballsFaced) * 100 : 0;
      });
      dotStats.sort((a, b) => b.dotPercentage - a.dotPercentage);
      return await formatPlayerStats(dotStats.slice(0, limit), prisma, '', 'dotPercentage');
    }
    ...
    case 'runs-without-cap': {
      ...
      const capWinners = new Set(Object.values(seasonMax).map((s) => s.playerId));
      ...
      return await formatPlayerStats(filtered.slice(0, limit), prisma, '_sum', 'runsScored');
    }
    ...
    case 'superstar-ducks': {
      ...
      return await formatPlayerStats(filtered.slice(0, limit), prisma, '_count', 'playerId');
    }
    ...
    case 'losing-wickets': {
      const matches = await prisma.playerMatchStats.findMany({
        select: { playerId: true, team: true, wickets: true, match: { select: { winner: true } } },
      });
      const losingWkts = {};
      matches.forEach((m) => {
        if (m.match.winner && m.team !== m.match.winner)
    ...
    case 'runs-before-100': {
      ...
      allStats.forEach((s) => {
        ...
        logs[s.playerId].push({ runs: s.runsScored, date: new Date(s.match.date).getTime() });
      });
      const runsBefore100 = [];
      for (const [playerId, playerLogs] of Object.entries(logs)) {
        ...
        if (hit100 && totalRuns > 0) runsBefore100.push({ playerId, totalRuns });
      }
    ...
    case 'season-runs':
      return await prisma.playerSeasonStats
        ...
        .then((res) =>
          res.map((r) => ({ player: r.player, value: r.totalRuns, context: `Season: ${r.season}` }))
        );
    case 'season-wickets':
      return await prisma.playerSeasonStats
        .findMany({
...
async function formatPlayerStats(groupedData, prisma, valuePath, valueKey) {
  ...
  return groupedData.map((d) => {
    ...
    return {
      player,
      value: typeof val === 'number' && !Number.isInteger(val) ? parseFloat(val.toFixed(2)) : val,
      raw: d,
    };
  });
}
