...
async function main() {
  ...
  log.info(`Found ${jsonFiles.length} match files. Pre-processing in memory...`);

  const existingMatches = new Set(
    (await prisma.match.findMany({ select: { cricsheetId: true } })).map((m) => m.cricsheetId)
  );
  ...
  for (const file of jsonFiles) {
    ...
    if (parsed.match.winner) parsed.match.winner = toShortName(parsed.match.winner);
    if (parsed.match.tossWinner) parsed.match.tossWinner = toShortName(parsed.match.tossWinner);
  ...
  if (missingPlayersSet.size > 0) {
    ...
    await prisma.player.createMany({ data: newPlayersData, skipDuplicates: true });

    const refreshedPlayers = await prisma.player.findMany({ select: { id: true, name: true } });
    for (const p of refreshedPlayers) {
      playerCache[p.name.toLowerCase()] = p.id;
    }
  }

  log.info(`Uploading ${parsedMatches.length} matches to database in parallel batches...`);
