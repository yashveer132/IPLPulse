...
async function validate() {
  ...
  const allFranchises = await prisma.franchise.findMany({
    orderBy: { name: "asc" },
    select: { name: true, shortName: true, titles: true },
  });
  allFranchises.forEach((f) =>
    console.log(
      `  ${f.shortName.padEnd(5)} ${f.name.padEnd(30)} Titles: ${f.titles}`,
    ),
  );

  console.log("\n📅 SEASON COVERAGE");
  console.log("-".repeat(50));
  const seasons = await prisma.playerSeasonStats.groupBy({
    by: ["season"],
    _count: { id: true },
    orderBy: { season: "asc" },
  });
  seasons.forEach((s) =>
    console.log(`  ${s.season}: ${s._count.id} player-season records`),
  );
  ...
  topPlayers.forEach((p, i) =>
    console.log(
      `  ${(i + 1).toString().padEnd(3)} ${p.name.padEnd(25)} ${p.role.padEnd(15)} ${p.nationality.padEnd(10)} ${p._count.matchStats} matches`,
    ),
  );
  ...
  h2hSamples.forEach((h) => {
    console.log(
      `  ${h.batter.name} vs ${h.bowler.name}: ${h.runsScored} runs, ${h.ballsFaced} balls, ${h.dismissals} dismissals, SR ${h.strikeRate.toFixed(1)}`,
    );
  });
  ...
  venueSamples.forEach((v) => {
    console.log(
      `  ${v.player.name} @ ${v.venue}: ${v.runsScored} runs, ${v.wickets} wkts`,
    );
  });
  ...
  crazySamples.forEach((c) => {
    const sr =
      c.deathOversBallsFaced > 0
        ? ((c.deathOversRunsScored / c.deathOversBallsFaced) * 100).toFixed(1)
        : 0;
    console.log(
      `  ${c.player.name}: ${c.deathOversRunsScored} death runs (SR ${sr}), ${c.powerplayWickets} PP wkts`,
    );
  });

  console.log("\n✅ DATA INTEGRITY CHECKS");
  console.log("-".repeat(50));
  const orphanH2H = await prisma.headToHeadStat.count({
    where: { batter: null },
  });
  const orphanVenue = await prisma.venueMasteryStat.count({
    where: { player: null },
  });
  console.log(
    `  Orphan H2H records (no batter):   ${orphanH2H === 0 ? "✅ PASS (0)" : "❌ FAIL (" + orphanH2H + ")"}`,
  );
  console.log(
    `  Orphan Venue records (no player):  ${orphanVenue === 0 ? "✅ PASS (0)" : "❌ FAIL (" + orphanVenue + ")"}`,
  );
  console.log(
    `  Players > 0:                       ${players > 0 ? "✅ PASS" : "❌ FAIL"}`,
  );
  console.log(
    `  Matches > 0:                       ${matches > 0 ? "✅ PASS" : "❌ FAIL"}`,
  );
  console.log(
    `  H2H > 0:                           ${headToHead > 0 ? "✅ PASS" : "❌ FAIL"}`,
  );
  console.log(
    `  Venue Mastery > 0:                 ${venueMastery > 0 ? "✅ PASS" : "❌ FAIL"}`,
  );
