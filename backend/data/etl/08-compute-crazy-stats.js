...
function normalizeVenue(v) {
  ...
  if (v.includes('Feroz Shah') || v.includes('Arun Jaitley')) return 'Arun Jaitley Stadium, Delhi';
  if (v.includes('Rajiv Gandhi')) return 'Rajiv Gandhi Int Stadium, Hyderabad';
  if (v.includes('Chepauk') || v.includes('Chidambaram')) return 'MA Chidambaram Stadium, Chennai';
  if (v.includes('Narendra Modi') || v.includes('Motera')) return 'Narendra Modi Stadium, Ahmedabad';
  if (v.includes('Sawai Mansingh')) return 'Sawai Mansingh Stadium, Jaipur';
  if (v.includes('PCA Stadium') || v.includes('Punjab Cricket')) return 'PCA Stadium, Mohali';
  ...
  if (v.includes('Maharashtra Cricket Association') || v.includes('MCA')) return 'MCA Stadium, Pune';
  ...
  if (v.includes('Dr. Y.S. Rajasekhara') || v.includes('ACA-VDCA')) return 'ACA-VDCA Stadium, Visakhapatnam';
  if (v.includes('Barabati')) return 'Barabati Stadium, Cuttack';
  if (v.includes('Himachal Pradesh') || v.includes('Dharamsala')) return 'HPCA Stadium, Dharamsala';
  if (v.includes('Vidarbha')) return 'VCA Stadium, Nagpur';
  if (v.includes('Kingsmead')) return 'Kingsmead, Durban';
  if (v.includes('Centurion') || v.includes('SuperSport')) return 'SuperSport Park, Centurion';
  ...
  if (v.includes('Mangaung') || v.includes('OUTsurance') || v.includes('Free State')) return 'Mangaung Oval, Bloemfontein';
  if (v.includes('Ekana') || v.includes('Lucknow')) return 'Ekana Cricket Stadium, Lucknow';
  if (v.includes('Barsapara') || v.includes('Guwahati')) return 'Barsapara Cricket Stadium, Guwahati';
  if (v.includes('Maharaja Yadavindra') || v.includes('Mullanpur')) return 'Maharaja Yadavindra Stadium, Mullanpur';
  return v;
}

async function main() {
  log.info('Starting Crazy Advanced Analytics Computation (Head-to-Head, Venue Mastery, Pressure Index)...');

  const jsonFiles = fs.readdirSync(RAW_DIR).filter((f) => f.endsWith('.json'));
  log.info(`Found ${jsonFiles.length} match files to parse.`);

  const players = await prisma.player.findMany({ select: { id: true, name: true, cricsheetId: true } });
  ...
  for (const file of jsonFiles) {
    const rawData = JSON.parse(fs.readFileSync(path.join(RAW_DIR, file), 'utf8'));
    ...
    inningsData.forEach((inningObj, inningIdx) => {
      ...
      overs.forEach((overObj) => {
        ...
        overObj.deliveries.forEach((ball) => {
          ...
          const isDot = runsScored === 0 && (ball.runs.extras === 0 || ball.extras?.legbyes || ball.extras?.byes) ? 1 : 0;
          ...
          if (!headToHead.has(h2hKey)) {
            headToHead.set(h2hKey, { 
              runsScored: 0, ballsFaced: 0, dismissals: 0, fours: 0, sixes: 0, dotBalls: 0,
              seasonDetails: {},
              phaseDetails: { Powerplay: { runs: 0, balls: 0, wkts: 0 }, Middle: { runs: 0, balls: 0, wkts: 0 }, Death: { runs: 0, balls: 0, wkts: 0 } },
              dismissalDetails: {}
            });
          }
          ...
          if (isWicket) {
            ...
            h2h.dismissalDetails[wktKind] = (h2h.dismissalDetails[wktKind] || 0) + 1;
          }

          const seasonStr = info.season?.toString() || 'Unknown';
          if (!h2h.seasonDetails[seasonStr]) {
            h2h.seasonDetails[seasonStr] = { runs: 0, balls: 0, wkts: 0, deliveries: [] };
          }
          ...
          h2h.seasonDetails[seasonStr].deliveries.push({
            date: info.dates && info.dates.length > 0 ? info.dates[0] : 'Unknown Date',
            venue: venue,
            over: overNum + 1,
            runs: runsScored,
          ...
          const phaseStr = isPowerplay ? 'Powerplay' : (isDeath ? 'Death' : 'Middle');
          ...
          if (!matchBatterStats.has(batterId)) {
            matchBatterStats.set(batterId, { runs: 0, balls: 0, fours: 0, sixes: 0, isOut: false });
          }
          ...
          if (!matchBowlerStats.has(bowlerId)) {
            matchBowlerStats.set(bowlerId, { runsConceded: 0, balls: 0, wickets: 0 });
          }
          ...
          if (isWicket) {
             ...
             if (wicketType !== 'run out' && wicketType !== 'retired hurt' && wicketType !== 'obstructing the field') {
                mBowl.wickets += 1;
             }
               ...
               ball.wickets[0].fielders.forEach(f => {
                 ...
                 if (fielderId) {
                   ...
                   if (!matchFieldingStats.has(fielderId)) {
                     matchFieldingStats.set(fielderId, { catches: 0, stumpings: 0, runOuts: 0 });
                   }
                   const fStat = matchFieldingStats.get(fielderId);
                   if (wicketType === 'caught' || wicketType === 'caught and bowled') fStat.catches += 1;
          ...
          if (!playerCrazy.has(batterId)) {
            playerCrazy.set(batterId, { deathRuns: 0, deathBalls: 0, deathWickets: 0, ppRunsConceded: 0, ppBallsBowled: 0, ppWickets: 0 });
          }
          if (!playerCrazy.has(bowlerId)) {
            playerCrazy.set(bowlerId, { deathRuns: 0, deathBalls: 0, deathWickets: 0, ppRunsConceded: 0, ppBallsBowled: 0, ppWickets: 0 });
          }
          ...
          if (isPowerplay) {
            ...
            if (!ball.extras?.wides && !ball.extras?.noballs) cBowl.ppBallsBowled += 1;
            if (isWicket) {
               const wicketType = ball.wickets[0].kind;
    ...
    playersInMatch.forEach(playerId => {
      ...
      if (!venueStats.has(vKey)) {
        venueStats.set(vKey, { 
          runsScored: 0, ballsFaced: 0, highestScore: 0, inningsBat: 0, notOuts: 0, 
          fours: 0, sixes: 0, fifties: 0, centuries: 0, 
          wickets: 0, runsConceded: 0, ballsBowled: 0, inningsBowl: 0, bestWickets: 0, bestRuns: 0,
          catches: 0, stumpings: 0, runOuts: 0, matchesPlayed: 0, matchLogs: []
        });
      }
      const vStat = venueStats.get(vKey);
      vStat.matchesPlayed += 1;

      const bStats = matchBatterStats.get(playerId) || { runs: 0, balls: 0, isOut: false };
      const wStats = matchBowlerStats.get(playerId) || { runsConceded: 0, wickets: 0, balls: 0 };
      const fStats = matchFieldingStats.get(playerId) || { catches: 0, stumpings: 0, runOuts: 0 };
    ...
    matchBowlerStats.forEach((stats, playerId) => {
      ...
      if (vStat) {
        ...
        if (stats.wickets > vStat.bestWickets || (stats.wickets === vStat.bestWickets && stats.runsConceded < vStat.bestRuns)) {
  ...
  const h2hArray = Array.from(headToHead.entries()).map(([key, stats]) => {
    ...
    const strikeRate = stats.ballsFaced > 0 ? (stats.runsScored / stats.ballsFaced) * 100 : 0;
    return { 
      batterId, 
      bowlerId, 
  ...
  const crazyArray = Array.from(playerCrazy.entries()).map(([playerId, stats]) => {
    return {
      playerId,
